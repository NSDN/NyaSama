...
public class TmpMain extends ApplicationAdapter {
    ...
    public void render() {
        ...
        /*renderer.begin();
        renderer.drawString(50f, 50f, 1f, Color.WHITE, "Time: " + Integer.toString(Time));
        renderer.end();*/
    ...
    private void MainShow() {
        if (!Flag) {
            ...
            if (Time < 100) {
                ...
                BackAlpha = 1.0f;
                if (Time < 200)
                    renderer.draw(Scene[1], DeviceWidth / 2, DeviceHeight / 2, (Time - 100) / 100f);
                else if (Time < 300)
                    renderer.draw(Scene[1], DeviceWidth / 2, DeviceHeight / 2, 1f);
                else if (Time < 400)
                    renderer.draw(Scene[1], DeviceWidth / 2, DeviceHeight / 2, (400 - Time) / 100f);

                else if (Time < 500)
                    renderer.draw(Scene[2], DeviceWidth / 2, DeviceHeight / 2, (Time - 400) / 100f);
                else if (Time < 600)
                    renderer.draw(Scene[2], DeviceWidth / 2, DeviceHeight / 2, 1f);
                else if (Time < 700)
                    renderer.draw(Scene[2], DeviceWidth / 2, DeviceHeight / 2, (700 - Time) / 100f);

                else if (Time < 800)
                    renderer.draw(Scene[3], DeviceWidth / 2, DeviceHeight / 2, (Time - 700) / 100f);
                else if (Time < 900)
                    renderer.draw(Scene[3], DeviceWidth / 2, DeviceHeight / 2, 1f);
                else if (Time < 1000)
                    renderer.draw(Scene[3], DeviceWidth / 2, DeviceHeight / 2, (1000 - Time) / 100f);

                else if (Time < 1100)
                    renderer.draw(Scene[4], DeviceWidth / 2, DeviceHeight / 2, (Time - 1000) / 100f);
                else if (Time < 1200)
                    renderer.draw(Scene[4], DeviceWidth / 2, DeviceHeight / 2, 1f);
                else if (Time < 1300)
                    renderer.draw(Scene[4], DeviceWidth / 2, DeviceHeight / 2, (1300 - Time) / 100f);

                else if (Time < 1400)
                    renderer.draw(Scene[5], DeviceWidth / 2, DeviceHeight / 2, (Time - 1300) / 100f);
                else if (Time < 1500)
                    renderer.draw(Scene[5], DeviceWidth / 2, DeviceHeight / 2, 1f);
                else if (Time < 1600)
                    renderer.draw(Scene[5], DeviceWidth / 2, DeviceHeight / 2, (1600 - Time) / 100f);

                else if (Time < 1700)
                    renderer.draw(Scene[0], DeviceWidth / 2, DeviceHeight / 2, (Time - 1600) / 100f);
                else if (Time < 1800)
                    renderer.draw(Scene[0], DeviceWidth / 2, DeviceHeight / 2, 1f);
                else if (Time < 1900) {
                    renderer.draw(Scene[0], DeviceWidth / 2, DeviceHeight / 2, (1900 - Time) / 100f);
            ...
            if (Time < 500) {
                ...
                renderer.draw(TexPlayer, PlayerX, PlayerY, 0);
                if (Gdx.input.isKeyPressed(Input.Keys.SHIFT_LEFT))
                    renderer.draw(TexSlow, PlayerX, PlayerY, Angle);

                renderer.drawString(50f, DeviceHeight - 50f, 1.8f, Color.WHITE, "Graze: " + Integer.toString(Graze));
                renderer.drawString(50f, DeviceHeight - 70f, 1.8f, Color.WHITE, "Miss:  " + Integer.toString(Miss));
                renderer.drawString(50f, DeviceHeight - 90f, 1.8f, Color.WHITE, "Score: " + Score);
    ...
    private void DrawBack() {
        if (Flag) {
            for (int i = 0; i <= 10; i++) {
                for (int j = 0; j <= 10; j++) {
                    renderer.draw(TexBack[0], BackA + 128f + 256f * i, BackA + 128f + 256f * j, BackAlpha);
                }
            }
        } else {
            for (int i = -1; i <= 10; i++) {
                for (int j = -1; j <= 10; j++) {
                    renderer.draw(TexBack[1], BackB + 256f + 512f * i, BackB + 256f + 512f * j, BackAlpha);
                }
            }
        }
    }
            ...
            for (int j = 0; j <= CtrlValueB; j++) {
                if (Bullets[i][j].IsEnabled)
                    if (Bullets[i][j].Direction) {
                        renderer.draw(TexBullet, Bullets[i][j].x, Bullets[i][j].y, Bullets[i][j].Rotate + Angle, Bullets[i][j].ScaleXY, Bullets[i][j].r, Bullets[i][j].g, Bullets[i][j].b, Bullets[i][j].a);
                    } else {
                        renderer.draw(TexBullet, Bullets[i][j].x, Bullets[i][j].y, Bullets[i][j].Rotate - Angle, Bullets[i][j].ScaleXY, Bullets[i][j].r, Bullets[i][j].g, Bullets[i][j].b, Bullets[i][j].a);
                    }
            }
        ...
        for (int i = 0; i <= CtrlValueA; i++)
            for (int j = 0; j <= CtrlValueB; j++) {
                if (Bullets[i][j].IsEnabled && Bullets[i][j].PreJudge(PlayerX, PlayerY, 500)) {
                    if (Bullets[i][j].Judge(PlayerX, PlayerY, Min, Max) == Bullet.JUDGE_AWAY && !Bullets[i][j].IsGrazed) {
                        Bullets[i][j].IsGrazed = true;
                    }
                    if (Bullets[i][j].Judge(PlayerX, PlayerY, Min, Max) == Bullet.JUDGE_GRAZE && Bullets[i][j].IsGrazed) {
                        Bullets[i][j].IsGrazed = false;
                        Graze++;
                        GRA.play();
                    }
                    if (Bullets[i][j].Judge(PlayerX, PlayerY, Min, Max) == Bullet.JUDGE_MISS) {
            ...
            for (int j = 0; j <= CtrlValueB; j++) {
                if (Bullets[i][j].IsEnabled) {
                    if (!Flag) {
                        ...
                        if (Utility.dist2(Bullets[i][j].x, Bullets[i][j].y, DeviceWidth / 2, DeviceHeight / 2) > Math.max(DeviceWidth, DeviceHeight))
                            Bullets[i][j].Init();
                    }
                }
            }
